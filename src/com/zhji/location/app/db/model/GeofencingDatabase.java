/*
 * Copyright (C) 2012 Sony Mobile Communications AB.
 * All rights, including trade secret rights, reserved.
 */

package com.zhji.location.app.db.model;

import com.zhji.location.app.db.DatabaseManager;

public class GeofencingDatabase extends BaseDatabase {
    /**
     * Table name
     */
    public static final String TABLE_NAME = "geofencing";

    /**
     * Column name
     */
    public static final String ID = "_id";

    public static final String LOCATION_ID = "location_id";

    public static final String TIMESTAMP = "timestamp";

    public static final String STATE = "state";

    /**
     * SQL Command: createTable
     */
    public static final String CREATE_TABLE = "CREATE TABLE " + TABLE_NAME + " (" + ID
            + " INTEGER PRIMARY KEY AUTOINCREMENT, " + LOCATION_ID + " INTEGER UNIQUE, "
            + TIMESTAMP + " INTEGER, " + STATE + " INTEGER);";

    /**
     * Constructor method
     * 
     * @param databaseManager
     */
    public GeofencingDatabase(final DatabaseManager databaseManager) {
        super(databaseManager, TABLE_NAME);
    }
}
